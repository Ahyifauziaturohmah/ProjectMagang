...
@php
    // Logika untuk menentukan mode Edit atau Tambah
    $isEdit = isset($magang);
    // Jika edit, arahkan ke route update. Jika tambah, arahkan ke url formmaganglist
    $actionUrl = $isEdit ? route('magang.update', $magang->id) : url('formmaganglist');
@endphp
  <div x-data="{ open: false }" class="flex flex-1">
    ...
    <div class="flex-1 p-20 overflow-auto">
      <h1 class="text-white text-3xl font-bold mb-6 leading-tight">
          {{ $isEdit ? 'Edit Data' : 'Tambahkan' }}<br>Anak Magang
      </h1>
      ...
      <form action="{{ $actionUrl }}" method="post" class="p-8 space-y-6 max-w-xl">
        ...
        <div>
          <label for="kontak" class="block text-white font-semibold mb-2">No. WhatsApp / Kontak:</label>
          <input type="text" id="kontak" name="kontak" 
            value="{{ old('kontak', $magang->contact->kontak ?? '') }}" required
