...
<body class="bg-[#1B7BA6] min-h-screen flex flex-col">
  ...
  <div x-data="{ open: false }" class="flex flex-1">
    ...
    <div class="flex-1 p-10 overflow-auto">
      <h1 class="text-white text-3xl font-bold mb-6 leading-tight">Daftar<br>Pengumpulan</h1>
      @if(count($data) > 0)
          <h2 class="text-white text-xl font-bold mb-6 leading-tight">Task {{ $data[0]->task->judul }}</h2>
      @endif
      ...
      <div class="bg-white rounded-lg shadow-lg overflow-x-auto border-4 border-[#6DC6FF]">
        <table class="min-w-full text-left border-collapse">
          ...
          <tbody>
            ...
            <tr class="bg-white">
              <td class="px-6 py-3 border border-gray-200">{{$item->user->name}}</td>
              <td class="px-6 py-3 border border-gray-200">{{$item->task->tenggat}}</td>
              <td class="px-6 py-3 border border-gray-200">
                <a href="{{ route('task.detail.pengumpulan', $item->id) }}" class="bg-pink-400 text-white px-4 py-1 rounded-full font-semibold text-sm hover:bg-pink-500">
                    Detail
                </a>
              </td>
            </tr>  
